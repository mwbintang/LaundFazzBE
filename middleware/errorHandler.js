const errorHandler = function (err, req, res, next) {
    ...
    if (err.name == 'Not login') {
        ...
        code = 401
        msg = err.name
    } else if (err.name == 'You are not admin' || err.name == 'You are not User' || err.name == 'You are not Employee' || err.name == "You're not allowed") {
        code = 403
        msg = err.name
        ...
        code = 404
        msg = 'Data Not Found'
    } else if (err.name == 'This News Already Added') {
        code = 400
        msg = err.name
    } else if (err.name == 'nodemailer error') {
        code = 403
        msg = 'nodemailer password wrong'
    }
